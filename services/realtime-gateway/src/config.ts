...
const envSchema = z.object({
  ...
  QWEN_OMNI_REALTIME_MODEL: z.string().default("qwen-audio-3.0-realtime-plus"),
  QWEN_OMNI_REALTIME_VOICE: z.string().default("longanqian"),
  QWEN_OMNI_REALTIME_TURN_DETECTION: z.enum(["server_vad", "semantic_vad", "smart_turn"]).default("smart_turn"),
  QWEN_OMNI_REALTIME_VAD_THRESHOLD: z.coerce.number().min(-1).max(1).default(0.2),
  QWEN_OMNI_REALTIME_SILENCE_MS: z.coerce.number().int().min(200).max(6000).default(500),
